...
MARKET_TICKERS = [
    {
        ...
        "name": "VIX (Fear Index)",
        "anomaly_rules": [
            {"type": "spike_above", "threshold": 20.0, "label": "moderate_fear", "weight": 0.30},
    ...
    },
    {
        ...
        "name": "S&P 500",
        "anomaly_rules": [
            {"type": "change_pct", "threshold": -1.0, "label": "sp500_decline", "weight": 0.25},
            {"type": "change_pct", "threshold": -2.0, "label": "sp500_selloff", "weight": 0.40},
            {"type": "change_pct", "threshold": -4.0, "label": "sp500_crash", "weight": 0.55},
            {"type": "change_pct", "threshold": 2.0, "label": "sp500_rally", "weight": 0.25},
        ],
    },
    {
        "symbol": "BZ=F",
        ...
        "anomaly_rules": [
            {"type": "change_pct", "threshold": 4.0, "label": "oil_significant_rise", "weight": 0.30},
            {"type": "change_pct", "threshold": 8.0, "label": "oil_spike", "weight": 0.45},
            {"type": "change_pct", "threshold": -4.0, "label": "oil_significant_drop", "weight": 0.30},
            {"type": "change_pct", "threshold": -8.0, "label": "oil_crash", "weight": 0.40},
            {"type": "spike_above", "threshold": 90.0, "label": "oil_elevated_level", "weight": 0.35},
    ...
    },
    {
        ...
        "name": "Gold",
        "anomaly_rules": [
            {"type": "change_pct", "threshold": 1.5, "label": "gold_bid_up", "weight": 0.25},
    ...
    },
    {
        ...
        "name": "EUR/USD",
        "anomaly_rules": [
            {"type": "change_pct", "threshold": 1.0, "label": "eurusd_move_up", "weight": 0.25},
            {"type": "change_pct", "threshold": -1.0, "label": "eurusd_move_down", "weight": 0.25},
            {"type": "change_pct", "threshold": 2.0, "label": "eurusd_volatility", "weight": 0.30},
            {"type": "change_pct", "threshold": -2.0, "label": "eurusd_volatility_down", "weight": 0.30},
        ],
    },
    {
        "symbol": "^TNX",
        ...
        "anomaly_rules": [
            {"type": "spike_above", "threshold": 4.5, "label": "yield_elevated", "weight": 0.30},
            {"type": "spike_above", "threshold": 5.0, "label": "yield_crisis", "weight": 0.45},
            {"type": "change_pct", "threshold": 5.0, "label": "yield_notable_move", "weight": 0.30},
    ...
    },
    {
        ...
        "anomaly_rules": [
            {"type": "change_pct", "threshold": 1.0, "label": "dollar_strength", "weight": 0.25},
            {"type": "change_pct", "threshold": -1.0, "label": "dollar_weakness", "weight": 0.25},
...
class MarketDataCollector:
    ...
    def _format_anomaly_headline(
        ...
        headlines = {
            "moderate_fear": f"[MARKET] VIX at {price:.1f} — market anxiety rising",
            ...
            "sp500_decline": f"[MARKET] S&P 500 down {abs(change_pct):.1f}% — notable decline",
            ...
            "sp500_rally": f"[MARKET] S&P 500 up {change_pct:.1f}% — strong rally",
            "oil_significant_rise": f"[MARKET] Brent crude up {change_pct:.1f}% to ${price:.2f} — supply concern signal",
            "oil_spike": f"[MARKET] Brent crude up {change_pct:.1f}% to ${price:.2f} — supply crisis signal",
            "oil_significant_drop": f"[MARKET] Brent crude down {abs(change_pct):.1f}% to ${price:.2f} — demand concern signal",
            "oil_crash": f"[MARKET] Brent crude down {abs(change_pct):.1f}% to ${price:.2f} — demand collapse signal",
            "oil_elevated_level": f"[MARKET] Brent crude at ${price:.2f} — elevated price level",
            "oil_crisis_level": f"[MARKET] Brent crude at ${price:.2f} — crisis price level",
            "gold_bid_up": f"[MARKET] Gold up {change_pct:.1f}% — safe-haven demand rising",
            ...
            "eurusd_move_up": f"[MARKET] EUR/USD up {change_pct:.1f}% — euro strengthening",
            "eurusd_move_down": f"[MARKET] EUR/USD down {abs(change_pct):.1f}% — euro weakening",
            "eurusd_volatility": f"[MARKET] EUR/USD moved {change_pct:+.1f}% — currency stress",
            "eurusd_volatility_down": f"[MARKET] EUR/USD moved {change_pct:+.1f}% — currency stress",
            "yield_elevated": f"[MARKET] 10Y yield at {price:.2f}% — elevated borrowing costs",
            "yield_crisis": f"[MARKET] 10Y yield at {price:.2f}% — borrowing cost crisis",
            "yield_notable_move": f"[MARKET] 10Y yield moved {change_pct:+.1f}% — bond market shift",
            "yield_spike": f"[MARKET] 10Y yield spiked {change_pct:.1f}% — bond market stress",
            "dollar_strength": f"[MARKET] US Dollar Index up {change_pct:.1f}% — dollar strengthening",
            "dollar_weakness": f"[MARKET] US Dollar Index down {abs(change_pct):.1f}% — dollar weakening",
